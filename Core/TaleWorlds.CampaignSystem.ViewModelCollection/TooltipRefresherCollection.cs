	...
	public static class TooltipRefresherCollection
	{
		...
		public static void RefreshSettlementTooltip(PropertyBasedTooltipVM propertyBasedTooltipVM, object[] args)
		{
			...
			if (CS$<>8__locals1.settlementAsParty != null)
			{
				...
				{
					TroopRoster troopRoster4 = TroopRoster.CreateDummyTroopRoster();
					foreach (MobileParty mobileParty5 in CS$<>8__locals1.settlement.Parties)
					{
						if (!mobileParty5.IsMainParty && !FactionManager.IsAtWarAgainstFaction(mobileParty5.MapFaction, CS$<>8__locals1.settlementAsParty.MapFaction))
						{
							for (int n = 0; n < mobileParty5.PrisonRoster.Count; n++)
							{
								TroopRosterElement elementCopyAtIndex2 = mobileParty5.PrisonRoster.GetElementCopyAtIndex(n);
					...
					for (int num4 = 0; num4 < CS$<>8__locals1.settlementAsParty.PrisonRoster.Count; num4++)
					{
						TroopRosterElement elementCopyAtIndex3 = CS$<>8__locals1.settlementAsParty.PrisonRoster.GetElementCopyAtIndex(num4);
					...
					if (!CS$<>8__locals1.settlement.IsHideout && (flag2 || flag))
					{
						...
						using (List<MobileParty>.Enumerator enumerator = list2.GetEnumerator())
						{
							while (enumerator.MoveNext())
							{
								...
								propertyBasedTooltipVM.AddProperty(mobileParty3.Name.ToString(), CampaignUIHelper.GetPartyNameplateText(mobileParty3), 0, TooltipProperty.TooltipPropertyFlags.None);
							}
							goto IL_BC9;
						}
					}
