...
namespace TaleWorlds.CampaignSystem.CraftingSystem
{
	public class CraftingOrder : ITrackableCampaignObject, ITrackableBase
	{
		...
		{
			WeaponDesignElement[] usedPieces = this._preCraftedWeaponDesignItemData.CraftedData.UsedPieces;
			for (int i = 0; i < usedPieces.Length; i++)
			{
				if (!usedPieces[i].IsValid)
				{
					return false;
				}
			}
		...
		public List<CraftingStatData> GetStatDataForItem(ItemObject itemObject, out WeaponComponentData weapon)
		{
			...
			foreach (KeyValuePair<CraftingTemplate.CraftingStatTypes, float> keyValuePair in itemObject.WeaponDesign.Template.GetStatDatas(num, weapon.ThrustDamageType, weapon.SwingDamageType))
			{
				...
				else if (keyValuePair.Key == CraftingTemplate.CraftingStatTypes.MissileDamage)
				{
					...
					{
						Debug.FailedAssert("Missile damage type is missing.", "C:\\Develop\\MB3\\Source\\Bannerlord\\TaleWorlds.CampaignSystem\\CraftingSystem\\CraftingOrder.cs", "GetStatDataForItem", 315);
					}
					list.Add(new CraftingStatData(textObject, (float)weapon.MissileDamage, keyValuePair.Value, keyValuePair.Key, damageTypes));
				}
		...
		{
			float num = 0f;
			Campaign.Current.MapSceneWrapper.GetHeightAtPoint(this.OrderOwner.CurrentSettlement.Position2D, ref num);
			return new Vec3(this.OrderOwner.CurrentSettlement.Position2D.x, this.OrderOwner.CurrentSettlement.Position2D.y, num, -1f);
		}
