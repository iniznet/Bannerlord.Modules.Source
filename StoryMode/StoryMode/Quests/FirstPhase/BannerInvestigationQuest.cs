	...
	public class BannerInvestigationQuest : StoryModeQuestBase
	{
		...
		protected override void SetDialogs()
		{
			...
			Campaign.Current.ConversationManager.AddDialogFlow(DialogFlow.CreateDialogFlow("hero_main_options", 150).BeginPlayerOptions().PlayerSpecialOption(new TextObject("{=qV1e0x8i}What is 'Neretzes' Folly'?", null), null)
				...
				.NpcLine(new TextObject("{=hFYG3lXw}Well, that's what some people call the great Battle of Pendraic in the year 1077.[ib:normal][if:convo_normal]", null), null, null)
		...
		private void SetNobleDialogs()
		{
			foreach (KeyValuePair<Hero, bool> keyValuePair in this._noblesToTalk)
			{
				if (!keyValuePair.Value)
				{
					...
					{
						textObject = new TextObject("{=L1V8L2N1}Yes. The Emperor Neretzes had offered to hire our warriors as mercenaries. I saw nothing wrong with that.[ib:confident3][if:convo_bored]", null);
					...
					else if (keyValuePair.Key.StringId == "lord_6_4")
					{
						...
						textObject4 = new TextObject("{=i5OOargr}Such are the fortunes of war. But what came afterwards... When word spread of what happened to our menfolk, the other clans - the Arkits in particular - knew we were weak. Our herds were raided. Anyone who protested was killed. Monchug did little to stop it. It taught us that valor will get you killed, but treachery will make you rich.[if:convo_angry]", null);
					}
					else if (keyValuePair.Key.StringId == "lord_6_16")
					{
						textObject = new TextObject("{=NGbgfPoP}I was there. Many of the Khuzaits went. Mostly the Khergit clan, who were hungry for glory, but I was also young and hungry for glory so I went along as well.[ib:demure2][if:convo_bored]", null);
						...
						textObject4 = new TextObject("{=4Baa38ra}We won, barely, with the help of the imperials, but the Khergits were mauled. Since then, the Khergits have been rather weak - and you know what happens to the weak. Still, no one told them to put all their eggs in one basket like that.[if:convo_bored2]", null);
					}
					else if (keyValuePair.Key.StringId == "lord_5_1")
					{
						textObject = new TextObject("{=Y6OJHdMH}I am a busy man, but there is always time to talk about the blessed battle of Pendraic.[if:convo_mocking_teasing][ib:confident3]", null);
						textObject2 = new TextObject("{=bmitHhDt}Our dear old beloved King Aeril, a wonderful man but with a heart perhaps just a might too tender, did not wish for us to go off to war. But then he disappeared and I, his son-in-law, ascended to the kingship. The clans cried out for war! They had a hundred years of crimes against them to avenge. I, a father to my people, gave them what they wanted.[if:convo_calm_friendly]", null);
						textObject3 = new TextObject("{=XEuhxpCX}Now, the Empire uses tricks and traps in war. No Battanian fears to meet an imperial soldier, man-to-man, but we thought it would be a good laugh to use their tricks against them. So we laid an ambush, on both sides of a wooded pass, and wouldn't you know? They marched right into it.[if:convo_happy]", null);
						textObject4 = new TextObject("{=aMBoh7gL}They turned and twisted as our arrows rained down upon them, like fish going frantic in a pond as you draw the net tighter. Then, when they were greatly discomfited, we took up our falxes and swords and reaped the harvest. Oh, there was some unpleasantness later with the Sturgians, about the spoils of war, but what a grand old day it was![if:convo_relaxed_happy]", null);
					}
					else if (keyValuePair.Key.StringId == "lord_5_3")
					{
						textObject = new TextObject("{=4Oc6MEvR}Ah... For any son of Battania, there will be no prouder moment in his life than that day. Any true son of Battania, anyway.[if:convo_calm_friendly][ib:hip]", null);
					...
					{
						textObject = new TextObject("{=BstbQV5Z}It was a tragedy that gnawed at the roots of all the great families of Calradia, even ours, so far away from the battle.[if:convo_annoyed][ib:normal2]", null);
					...
					else if (keyValuePair.Key.StringId == "lord_3_3")
					{
						...
						textObject4 = new TextObject("{=Xg6jhfPt}Nimr returned, in well-deserved glory. A daughter of the Banu Qild took an interest in him, and they had a secret affair, as the youth sometimes do. As heroes do. But Nimr's acts wounded the Qildi's pride. They kidnapped him, slew him and hung him in a cage in their market. We will forgive the Empire and the Vlandians. The Qildis... better not ask me that.[if:convo_contemptuous][ib:confident]", null);
					}
					else if (keyValuePair.Key.StringId == "lord_3_5")
					{
						textObject = new TextObject("{=D4MuSwRB}A sad day for the Banu Qild. But we had our vengeance.[if:convo_stern][ib:hip]", null);
						textObject2 = new TextObject("{=Qb0wsarP}There was a warrior named Nimr, of the Bani Sarran. He was brave, but arrogant. Of course the young people loved him. He wanted to lead men to fight with the Empire, and though there was no gain for us, Unqid let him. Unqid can be weak sometimes.[if:convo_thinking]", null);
						textObject3 = new TextObject("{=flkmzKDz}Many Qildi youth went with him. They died in their hundreds. And there was no gain - except for Nimr, who for some reason people considered a hero. It was despicable how they fawned on him.[if:convo_annoyed]", null);
					...
					else if (keyValuePair.Key.StringId == "lord_2_1")
					{
						...
						textObject4 = new TextObject("{=Ri6NmpEE}I will never forget when a messenger ran to tell me that my father was dead. But I knew I must swallow my grief, because now I was king. I rode down into the ruins of the imperial camp to take their banner as a trophy, my inheritance won by my father and passed down to me. Oh, some of the boyars were insubordinate. But I have since showed them that I am master.[if:convo_contemptuous]", null);
					}
					else if (keyValuePair.Key.StringId == "lord_2_3")
					{
						textObject = new TextObject("{=X3pDRmjK}A victory, won by my father, claimed by Raganvad.[if:convo_annoyed][ib:warrior2]", null);
						textObject2 = new TextObject("{=scjogyUD}Old king Vadinslav was brave enough. He led us all into battle. I stood at my father's side as we faced the imperials eye-to-eye over the tops of our shields. It was like any battle where shield walls meet - thrust and push, struggling to stay on your feet, but you can't really describe it. Let's just say it's the kind of battle that Sturgians usually win.[ib:confident2]", null);
					...
					else if (keyValuePair.Key.StringId == "lord_2_5")
					{
						...
						textObject4 = new TextObject("{=ez4rbGmx}Insults his most powerful vassal and then insults his most valued ally. A fine day's work, wouldn't you say? But he has grown wiser since, though no more pleasant to spend time with.[ib:demure2]", null);
					}
					else if (keyValuePair.Key.StringId == "lord_1_1")
					{
						textObject = new TextObject("{=yRAl8YRL}Yes. I was a junior officer on Neretzes' staff. People say much about the battle that betrays a lack of understanding, of Neretzes and of the circumstances he faced.[if:convo_undecided_closed][ib:closed]", null);
						textObject2 = new TextObject("{=bdbZLJf3}Neretzes had an obligation to avenge the Battanian attacks on our land. He marched out, with all the forces he could gather. The Vlandians betrayed us, but that's what you expect from honorless barbarians. Fortune favored the enemy. What matters is that we did what honor required.[if:convo_thinking]", null);
						textObject3 = new TextObject("{=CRmUZvaW}Perhaps Neretzes was rash, sending our infantry up into the hills to storm the Battanian fort. But he thought he could grab the pass quickly, before the enemy had time to reinforce it. If he had made the other wager and that turned out to be wrong, people would say he was hesitant.[ib:demure2]", null);
					...
					{
						textObject = new TextObject("{=HL9lzfjY}Of course I can. You know my name, whose son I am.[if:convo_beaten][ib:closed]", null);
