...
namespace StoryMode.Quests.TutorialPhase
{
	public class FindHideoutTutorialQuest : StoryModeQuestBase
	{
		...
		{
			MobileParty mobileParty = BanditPartyComponent.CreateBanditParty("radagos_raider_party_" + number, this._hideout.OwnerClan, this._hideout.Hideout, isBanditBossParty);
		...
		protected override void SetDialogs()
		{
			...
			Campaign.Current.ConversationManager.AddDialogFlow(DialogFlow.CreateDialogFlow("start", 1000015).NpcLine(new TextObject("{=R3CnF55p}So... Who's this that comes through my place of business, killing my employees?[if:convo_confused_voice][ib:warrior2]", null), null, null).Condition(new ConversationSentence.OnConditionDelegate(this.bandit_hideout_boss_fight_start_on_condition))
				.PlayerLine(new TextObject("{=itRoeaJf}We heard you took our little brother and sister. Where are they?", null), null)
				.NpcLine(new TextObject("{=eVgAY7ts}Good Heaven, I'll need a better description than that. My men have harvested dozens of little brats in this region. Quite good hunting grounds! Already sent most of them south to a slave market I know, though.[ib:hip][if:convo_snide_voice]", null), null, null)
				.NpcLine(new TextObject("{=wWLnZ6G4}Since your hunt for your kin is fruitless, how about you clear off and save your own lives? Either that or I force you to lick up all the blood you've spilled here with your tongues. Or... You and I could settle this, one on one.[if:convo_angry_voice]", null), null, null)
				.BeginPlayerOptions()
				...
				.EndPlayerOptions()
				.CloseDialog(), this);
			Campaign.Current.ConversationManager.AddDialogFlow(DialogFlow.CreateDialogFlow("start", 1000010).NpcLine(new TextObject("{=ZhZ7MCeh}Well. I recognize defeat when I see it. If I'm going to be your captive, let me introduce myself. I'm Radagos.[ib:weary2][if:convo_uncomfortable_voice]", null), null, null).Condition(new ConversationSentence.OnConditionDelegate(this.radagos_meeting_conversation_condition))
				.NpcLine(new TextObject("{=w0CUaEU7}You haven't cut my throat yet, which was a wise move. I'm sure I can find a way to be worth more to you alive than dead.[if:convo_calm_friendly]", null), null, null)
				...
				.NpcLine(new TextObject("{=FWSwngVX}Shall we get on the road? Remember - if I drop dead of exhaustion, or drown in some river, that's it for your little dears. I don't expect a cozy palanquin, now, but you'd best not make it too hard a trip for me.[if:convo_uncomfortable_voice]", null), null, null)
				.Consequence(new ConversationSentence.OnConsequenceDelegate(this.radagos_meeting_conversation_consequence))
				.CloseDialog(), this);
			Campaign.Current.ConversationManager.AddDialogFlow(DialogFlow.CreateDialogFlow("start", 1000020).NpcLine(new TextObject("{=qp2zYfua}I was hoping to find more treasure here, but I think business wasn't going too well for {RADAGOS.NAME} and his gang.[ib:closed2][if:convo_pondering]", null), null, null).Condition(new ConversationSentence.OnConditionDelegate(this.brother_farewell_conversation_condition))
				.NpcLine(new TextObject("{=J4qetbZb}I found this strange looking metal piece though. It doesn't look too valuable, but it could be the artifact {TACTEOS.NAME} was talking about. Maybe we can sell it to one of the noble clans for a hefty price.[if:convo_astonished]", null), null, null)
				.PlayerLine(new TextObject("{=OffNcRby}All right then. Let's get on the road.", null), null)
				.NpcLine(new TextObject("{=j9f9Ts7i}I have a better idea. We would have a better chance if we split up now. I'll take {RADAGOS.NAME} and go find the slaver market and look for a way to free the children. However we must be careful not to endanger their lives and it could be better to just buy them. We need to have our purses full for that though.[if:convo_normal]", null), null, null)
				.NpcLine(new TextObject("{=fp6QBO7l}I'll need to take these men with us. {RADAGOS.NAME} is a slippery one. I don't want him getting away.[if:convo_confused_voice]", null), null, null)
				.PlayerLine(new TextObject("{=RJ9NbuYr}So you want me to raise the money to ransom the little ones?", null), null)
				.NpcLine(new TextObject("{=4OUnPjZc}Indeed. You'll have to find a way to do that. Maybe this bronze thing can help.[if:convo_empathic_voice]", null), null, null)
				.NpcLine(new TextObject("{=5soUEFEJ}{TACTEOS.NAME} said it could be worth a fortune to the right person, if you manage not to get killed. If he's telling the truth, you must be careful. Never reveal that you have it. Try to understand its value, and how it can be sold.[if:convo_pondering]", null), null, null)
				.NpcLine(new TextObject("{=jPKIN2r4}One more thing. When you are talking to nobles and other people of importance, make sure you present yourself as someone from a distant but distinguished family.[if:convo_thinking]", null), null, null)
				.NpcLine(new TextObject("{=GVMGXfxS}You can use our family name if you like or make up a new one. You will have a better chance of obtaining an audience with nobles and it'll be easier for me to find you by asking around.[if:convo_normal]", null), null, null)
				.Consequence(new ConversationSentence.OnConsequenceDelegate(this.SelectClanName))
				.NpcLine(new TextObject("{=qIltCuBe}Get on the road now. Once I locate the little ones, I'll come find you.[ib:normal][if:convo_calm_friendly]", null), null, null)
