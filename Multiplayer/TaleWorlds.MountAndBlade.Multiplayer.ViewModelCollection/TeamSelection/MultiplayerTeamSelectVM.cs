...
namespace TaleWorlds.MountAndBlade.Multiplayer.ViewModelCollection.TeamSelection
{
	public class MultiplayerTeamSelectVM : ViewModel
	{
		...
		public MultiplayerTeamSelectVM(Mission mission, Action<Team> onChangeTeamTo, Action onAutoAssign, Action onClose, IEnumerable<Team> teams, string gamemode)
		{
			...
			Debug.Print("MultiplayerTeamSelectVM 1", 0, 12, 17179869184UL);
			...
			Debug.Print("MultiplayerTeamSelectVM 2", 0, 12, 17179869184UL);
			this.IsRoundCountdownAvailable = this._gameMode.IsGameModeUsingRoundCountdown;
			Debug.Print("MultiplayerTeamSelectVM 3", 0, 12, 17179869184UL);
			Team team = teams.FirstOrDefault((Team t) => t.Side == -1);
			this.TeamSpectators = new TeamSelectTeamInstanceVM(missionBehavior, team, null, null, onChangeTeamTo, false);
			Debug.Print("MultiplayerTeamSelectVM 4", 0, 12, 17179869184UL);
			Team team2 = teams.FirstOrDefault((Team t) => t.Side == 1);
			BasicCultureObject basicCultureObject = MBObjectManager.Instance.GetObject<BasicCultureObject>(MultiplayerOptionsExtensions.GetStrValue(14, 0));
			...
			Debug.Print("MultiplayerTeamSelectVM 5", 0, 12, 17179869184UL);
			Team team3 = teams.FirstOrDefault((Team t) => t.Side == 0);
			basicCultureObject = MBObjectManager.Instance.GetObject<BasicCultureObject>(MultiplayerOptionsExtensions.GetStrValue(15, 0));
			...
			Debug.Print("MultiplayerTeamSelectVM 6", 0, 12, 17179869184UL);
			if (GameNetwork.IsMyPeerReady)
			{
				this._missionPeer = PeerExtensions.GetComponent<MissionPeer>(GameNetwork.MyPeer);
				this.IsCancelDisabled = this._missionPeer.Team == null;
			}
			Debug.Print("MultiplayerTeamSelectVM 7", 0, 12, 17179869184UL);
			this.RefreshValues();
			Debug.Print("MultiplayerTeamSelectVM 8", 0, 12, 17179869184UL);
		}
