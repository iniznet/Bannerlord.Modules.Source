...
namespace TaleWorlds.MountAndBlade.Multiplayer.ViewModelCollection.ClassLoadout
{
	public class HeroClassVM : ViewModel
	{
		...
		public HeroClassVM(Action<HeroClassVM> onSelect, Action<HeroPerkVM, MPPerkVM> onPerkSelect, MultiplayerClassDivisions.MPHeroClass heroClass, bool useSecondary)
		{
			...
			this.CultureColor = Color.FromUint(this.UseSecondary ? heroClass.Culture.Color2 : heroClass.Culture.Color);
			this._gameMode = Mission.Current.GetMissionBehavior<MissionMultiplayerGameModeBaseClient>();
			this.Gold = (this._gameMode.IsGameModeUsingCasualGold ? this.HeroClass.TroopCasualCost : ((this._gameMode.GameType == 4) ? this.HeroClass.TroopBattleCost : this.HeroClass.TroopCost));
			this.InitPerksList();
			int intValue = MultiplayerOptionsExtensions.GetIntValue(20, 0);
			this.IsNumOfTroopsEnabled = !this._gameMode.IsInWarmup && intValue > 0;
			if (this.IsNumOfTroopsEnabled)
			{
				this.NumOfTroops = MPPerkObject.GetTroopCount(heroClass, intValue, MPPerkObject.GetOnSpawnPerkHandler(this._perks.Select((HeroPerkVM p) => p.SelectedPerk)));
			}
			this.UpdateEnabled();
			this.RefreshValues();
		}
