...
namespace TaleWorlds.MountAndBlade.Multiplayer.ViewModelCollection.ClassLoadout
{
	public class MultiplayerClassLoadoutVM : ViewModel
	{
		...
		public MultiplayerClassLoadoutVM(MissionMultiplayerGameModeBaseClient gameMode, Action<MultiplayerClassDivisions.MPHeroClass> onRefreshSelection, MultiplayerClassDivisions.MPHeroClass initialHeroSelection)
		{
			...
			int num = ((initialHeroSelection != null) ? ((!gameMode.IsGameModeUsingCasualGold) ? ((gameMode.GameType == 4) ? initialHeroSelection.TroopBattleCost : initialHeroSelection.TroopCost) : initialHeroSelection.TroopCasualCost) : 0);
			...
			if (Mission.Current.HasMissionBehavior<MissionMultiplayerSiegeClient>())
			{
				...
				this.IsAttacker = team.Side == 1;
			}
			this.RefreshValues();
			this._isTeammateAndEnemiesRelevant = Mission.Current.GetMissionBehavior<MissionMultiplayerGameModeBaseClient>().IsGameModeTactical && !Mission.Current.HasMissionBehavior<MissionMultiplayerSiegeClient>() && Mission.Current.GetMissionBehavior<MissionMultiplayerGameModeBaseClient>().GameType != 4;
		...
		private void UpdateSpawnAndTimerLabels()
		{
			...
			if (this._missionMultiplayerGameMode.RoundComponent != null)
			{
				...
				if (this._missionMultiplayerGameMode.IsRoundInProgress && (this._missionMultiplayerGameMode is MissionMultiplayerGameModeFlagDominationClient && this._missionMultiplayerGameMode.GameType == 6) && PeerExtensions.GetComponent<MissionPeer>(GameNetwork.MyPeer) != null)
				{
		...
		private void OnRefreshTeamMembers()
		{
			...
			{
				if (PeerExtensions.GetComponent<MissionPeer>(PeerExtensions.GetNetworkPeer(missionPeer)) != null && this._playerTeam != null && missionPeer.Team == this._playerTeam)
				{
