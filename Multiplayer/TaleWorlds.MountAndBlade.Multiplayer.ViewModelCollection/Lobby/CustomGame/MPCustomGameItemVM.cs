...
namespace TaleWorlds.MountAndBlade.Multiplayer.ViewModelCollection.Lobby.CustomGame
{
	public class MPCustomGameItemVM : ViewModel
	{
		...
		private void UpdatePremadeGameInfo()
		{
			...
			if (this.PremadeGameInfo.PremadeGameType == 1)
			{
				this.PremadeMatchTypeText = new TextObject("{=YNkPy4ta}Clan Match", null).ToString();
				return;
			}
			if (this.PremadeGameInfo.PremadeGameType == null)
			{
		...
		private List<TooltipProperty> GetLoadedModulesTooltipProperties()
		{
			...
			if (this.GameServerInfo != null)
			{
				...
				{
					list.Add(new TooltipProperty(string.Empty, new TextObject("{=JXyxj1J5}Modules", null).ToString(), 1, false, 4096));
					...
					foreach (ModuleInfoModel moduleInfoModel in this.GameServerInfo.LoadedModules)
					{
						...
						list.Add(new TooltipProperty(moduleInfoModel.Name, text2, 0, false, 0));
					}
				}
				...
				list.Add(new TooltipProperty("", textObject.ToString(), -1, false, 0));
				if (this.IsCommunityServer)
				{
					list.Add(new TooltipProperty("", string.Empty, 0, false, 1024));
					...
					list.Add(new TooltipProperty(string.Empty, textObject2.ToString(), -1, false, 0)
					{
