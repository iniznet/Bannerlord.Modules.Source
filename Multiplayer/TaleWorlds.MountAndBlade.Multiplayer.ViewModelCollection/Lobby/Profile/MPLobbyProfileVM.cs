...
namespace TaleWorlds.MountAndBlade.Multiplayer.ViewModelCollection.Lobby.Profile
{
	public class MPLobbyProfileVM : ViewModel
	{
		...
		public void RefreshRecentGames(MBReadOnlyList<MatchInfo> recentGames)
		{
			...
			int num = Math.Min(3, orderedEnumerable.Count<MatchInfo>());
			for (int i = 0; i < num; i++)
			{
		...
		private void OnEnabledChanged()
		{
			...
			{
				PlatformServices.Instance.CheckPermissionWithUser(4, this.PlayerInfo.Player.ProvidedID, delegate(bool hasBannerlordIDPrivilege)
				{
