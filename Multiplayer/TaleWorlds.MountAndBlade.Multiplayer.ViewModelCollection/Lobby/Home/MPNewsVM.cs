...
namespace TaleWorlds.MountAndBlade.Multiplayer.ViewModelCollection.Lobby.Home
{
	public class MPNewsVM : ViewModel
	{
		...
		private void RefreshNews()
		{
			...
			List<IGrouping<int, NewsItem>> list = (from i in (from i in this._newsManager.NewsItems.Where((NewsItem n) => n.Feeds.Any((NewsType t) => t.Type == 2) && !string.IsNullOrEmpty(n.Title) && !string.IsNullOrEmpty(n.NewsLink) && !string.IsNullOrEmpty(n.ImageSourcePath)).ToList<NewsItem>()
					group i by i.Feeds.First((NewsType t) => t.Type == 2).Index).ToList<IGrouping<int, NewsItem>>()
				orderby i.Key
