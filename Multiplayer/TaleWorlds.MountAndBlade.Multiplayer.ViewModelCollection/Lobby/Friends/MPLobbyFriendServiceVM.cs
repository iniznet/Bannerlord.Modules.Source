...
namespace TaleWorlds.MountAndBlade.Multiplayer.ViewModelCollection.Lobby.Friends
{
	public class MPLobbyFriendServiceVM : ViewModel
	{
		...
		public MPLobbyFriendServiceVM(IFriendListService friendListService, Action<PlayerId> onFriendRequestAnswered, Action<MPLobbyPlayerBaseVM> activatePlayerActions)
		{
			...
			this._isInGameFriendsRelevant = friendListServiceType != null && friendListServiceType != 1 && friendListServiceType != 2 && friendListServiceType != 9;
		...
		private void ProcessFriendRequest(PlayerId friendId)
		{
			...
			PlatformServices.Instance.CheckPrivilege(3, false, delegate(bool privilegeResult)
			{
				if (!privilegeResult)
				{
		...
		{
			PlatformServices.Instance.CheckPrivilege(4, true, delegate(bool result)
			{
				if (result)
				{
