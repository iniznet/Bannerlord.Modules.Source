	...
	public class MissionMultiplayerSpectatorHUDVM : ViewModel
	{
		...
		private void UpdateDynamicProperties()
		{
			...
			if (this.SpectatedPlayerNeutrality > 0 && this._spectatedAgent != null)
			{
				...
				EquipmentIndex wieldedItemIndex = this._spectatedAgent.GetWieldedItemIndex(0);
				EquipmentIndex wieldedItemIndex2 = this._spectatedAgent.GetWieldedItemIndex(1);
				int num = -1;
				if (wieldedItemIndex != -1 && this._spectatedAgent.Equipment[wieldedItemIndex].CurrentUsageItem != null)
				{
					...
					{
						int ammoAmount = this._spectatedAgent.Equipment.GetAmmoAmount(wieldedItemIndex);
					...
					{
						bool flag = this._spectatedAgent.Equipment[wieldedItemIndex].CurrentUsageItem.WeaponClass == 16;
						num = this._spectatedAgent.Equipment.GetAmmoAmount(wieldedItemIndex) + (int)(flag ? this._spectatedAgent.Equipment[wieldedItemIndex].Ammo : 0);
					}
				}
				if (wieldedItemIndex2 != -1 && this._spectatedAgent.Equipment[wieldedItemIndex2].CurrentUsageItem != null)
				{
		...
		internal void OnSpectatedAgentFocusIn(Agent followedAgent)
		{
			...
			if (missionPeer3 != null)
			{
				...
				TargetIconType targetIconType = ((mpheroClassForPeer != null) ? mpheroClassForPeer.IconType : (-1));
