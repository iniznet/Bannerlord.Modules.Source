	...
	public class CommanderInfoVM : ViewModel
	{
		public CommanderInfoVM(MissionRepresentativeBase missionRepresentative)
		{
			...
			if (this._gameMode != null)
			{
				...
				this.UsePowerComparer = this._gameMode.GameType == 4 && this._gameMode.ScoreboardComponent != null;
		...
		internal void Tick(float dt)
		{
			...
			if (this._allyTeam != null && this.UsePowerComparer)
			{
				...
				this.AllyMemberCount = ((this._allyTeam.Side == 1) ? count : count2);
				this.EnemyMemberCount = ((this._allyTeam.Side == 1) ? count2 : count);
				int num = ((this._allyTeam.Side == 1) ? this._attackerTeamInitialMemberCount : this._defenderTeamInitialMemberCount);
				Team allyTeam = this._allyTeam;
				int num2 = ((allyTeam != null && allyTeam.Side == 1) ? this._defenderTeamInitialMemberCount : this._attackerTeamInitialMemberCount);
				if (num2 == 0 && num == 0)
				{
					this.PowerLevelComparer.Update(1.0, 1.0, 1.0, 1.0);
					return;
		...
		private void InitCapturePoints()
		{
			if (this._commanderInfo != null)
			{
				...
				if (flag)
				{
					...
					{
						CapturePointVM capturePointVM = new CapturePointVM(flagCapturePoint, 17 + flagCapturePoint.FlagIndex);
						this.HandleAddNewCapturePoint(capturePointVM);
					}
		...
		private void HandleAddNewCapturePoint(CapturePointVM capturePointVM)
		{
			...
			if (team != null && (team.Side == -1 || team.Side == 2))
			{
				team = null;
			}
