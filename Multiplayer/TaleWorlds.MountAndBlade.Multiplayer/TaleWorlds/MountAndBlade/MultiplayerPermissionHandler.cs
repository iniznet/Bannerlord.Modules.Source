...
namespace TaleWorlds.MountAndBlade
{
	public class MultiplayerPermissionHandler : UdpNetworkComponent
	{
		...
		private void HandleServerEventInitializeLobbyPeer(GameNetworkMessage baseMessage)
		{
			...
			{
				if (PlatformServices.Instance.RegisterPermissionChangeEvent(initializeLobbyPeer.ProvidedId, 1, new PermissionChanged(this.TextPermissionChanged)))
				{
					this._registeredEvents[new ValueTuple<PlayerId, Permission>(initializeLobbyPeer.ProvidedId, 1)] = true;
				}
				if (PlatformServices.Instance.RegisterPermissionChangeEvent(initializeLobbyPeer.ProvidedId, 2, new PermissionChanged(this.VoicePermissionChanged)))
				{
					this._registeredEvents[new ValueTuple<PlayerId, Permission>(initializeLobbyPeer.ProvidedId, 2)] = true;
				}
			}
		}
		...
		public override void OnPlayerDisconnectedFromServer(NetworkCommunicator networkPeer)
		{
			...
			if (PlatformServices.Instance.UnregisterPermissionChangeEvent(networkPeer.VirtualPlayer.Id, 1, new PermissionChanged(this.TextPermissionChanged)))
			{
				bool flag;
				this._registeredEvents.TryRemove(new ValueTuple<PlayerId, Permission>(networkPeer.VirtualPlayer.Id, 1), out flag);
			}
			if (PlatformServices.Instance.UnregisterPermissionChangeEvent(networkPeer.VirtualPlayer.Id, 2, new PermissionChanged(this.VoicePermissionChanged)))
			{
				bool flag;
				this._registeredEvents.TryRemove(new ValueTuple<PlayerId, Permission>(networkPeer.VirtualPlayer.Id, 2), out flag);
			}
		}
		...
		private void TextPermissionChanged(PlayerId targetPlayerId, Permission permission, bool hasPermission)
		{
			foreach (NetworkCommunicator networkCommunicator in GameNetwork.NetworkPeers)
			{
				...
				{
					PeerExtensions.GetComponent<MissionPeer>(networkCommunicator);
